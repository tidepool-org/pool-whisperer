      ...
      async.waterfall(
        ...
        function(err, hashPair) {
          ...
          dataBroker.getDataStream(dataStreamId, function(err, stream){
            ...
            var retVal = rx.Observable.fromArray(stream);
            retVal = dataHelpers.convertBasal(retVal);
            retVal = dataHelpers.convertBolus(retVal);
            retVal.toArray().subscribe(
              function(data) {
                res.send(200, data);
              },
              function(error) {
                log.warn(error, 'Error retrieving data for user[%s]', req.params.userid);
                res.send(500);
              }
            );
          });
