...
module.exports = function seagullService(userApiClient, seagullClient, serverPlatformClient, dataBroker) {
  function createServer(serverConfig) {
    ...
    retVal.get('/:userid', checkToken, function(req, res, next){
      ...
      async.waterfall(
        [
          function(cb) {
            serverPlatformClient.canViewData(presentUser, requestedUser, cb);
          },
          function(canView, cb) {
            if (canView) {
